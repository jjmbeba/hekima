...
const AddEventForm = () => {
  ...
  const { mutate, isPending: isAddEventPending } = useMutation({
    mutationKey: ["events"],
    ...
    onSuccess: (createdEvent) => {
      toast.success(`Event '${createdEvent.name}' added successfully`);
      router.refresh();
      form.reset();
    },
    onError: (error) => {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Button type="submit" disabled={isAddEventPending}>
          {isAddEventPending && (
